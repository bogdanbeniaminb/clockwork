...
class Clockwork
{
    ...
    {
        if (! $this->config['enable'] && ! $this->config['collect_data_always']) {
            return $this->psrResponse;
        }

        if (! $this->clockwork->shouldCollect()->filter($this->incomingRequest())) {
            return $this->psrResponse;
        }
        if (! $this->clockwork->shouldRecord()->filter($this->clockwork->request())) {
            return $this->psrResponse;
        }
    ...
    {
        if (! $this->config['enable'] || $this->headersSent) {
            return;
        }
    ...
    protected function response($data = null, $status = null)
    {
        ...
        if ($this->psrResponse) {
            if ($status) {
                $this->psrResponse = $this->psrResponse->withStatus($status);
            }
